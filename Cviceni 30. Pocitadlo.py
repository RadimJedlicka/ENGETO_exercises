...
# cista_slova = text.lower()
# cista_slova = text.strip(".:;,")
# cista_slova = text.split()
# print(cista_slova)
...
for slovo in cista_slova:
    if slovo not in pocet_slov:
        pocet_slov.setdefault(slovo, 1)
        # pocet_slov[slovo] = 1               # ekvivalentni zapis
    else:
        pocet_slov[slovo] += 1
        # pocet_slov[slovo] =  pocet_slov[slovo] + 1      # ekvivaletni zapis
print(pocet_slov)

# 5 nejcastejsich slov
pet_nejcastejsich = sorted(pocet_slov.values(), reverse=True)[:5]
print(pet_nejcastejsich)

# slova s nejcastejsim vyskytem
vysledky = []
for slova in pocet_slov:
    if pocet_slov[slova] in pet_nejcastejsich:
        vysledky.append((slova, pocet_slov[slova]))
...
vysledky = sorted(vysledky, key=take_second, reverse=True)
